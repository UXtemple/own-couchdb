...
test('server', t => {
  ...
  t.deepEquals(createHandler.args[0][0], handlerArgs, `creates handler passing the endpoint`);
  ...
  t.equals(console.log.args[0][0], `CouchDB own running on port http://localhost:${config.port} against ${config.endpoint}`, `logs the server info to the console`);
